    ...
    */
    ...
    if (!empty($_POST)) {
        ...
        foreach($_POST as $key => $value){
            if($key == "name"){
                if($value == $pallet_name){
                    $pallet_new_name = $pallet_name;
                    continue;
                }

                if($value == "Pallet"){
                    $pallet_new_name = "PALLET_PLACEHOLDER_NAME";
                }
        ...
        if(count($quantities_by_cat) == 0 && empty($errors)){
            ...
            if($pallet_new_name == "PALLET_PLACEHOLDER_NAME"){
                $pallet_new_name = "Pallet " . $palletEventId;
                $name_counter = 1;
                while(!pallet_name_unique($pallet_new_name)){
                    $name_counter++;
                    $pallet_new_name = "Pallet " . $palletEventId . " (" . $name_counter . ")";
                }
            }
        ...
        foreach($palletCounts as $palletCount){
            $categories_by_row[] = $palletCount->getItemCategory();
            $quantities_by_row[] = $palletCount->getQuantity();
            $expirations_by_row[] = $palletCount->getExpiration();
        }
    }    
    ...
    <main>
        <div class="report-container">
            ...
            <div class="report-section">
                <h2>Modify Pallet: <?= $thePallet->getName() ?></h2>              
